...
def main():
    ...
    ax1 = axes.flat[1]
    im = ax1.imshow(conn_tap,vmin=-1, vmax=1)
    ax1.set_title('Finger Tap')
    #ax1.set_xticks(range(conn_mat.shape[0]))
    #ax1.set_yticks(range(conn_mat.shape[1]))

    ax2 = axes.flat[2]
    im = ax2.imshow(conn_reach,vmin=-1, vmax=1)
    ax2.set_title('Reach')
    #ax2.set_xticks(range(conn_mat.shape[0]))
    #ax2.set_yticks(range(conn_mat.shape[1]))

    ax3 = axes.flat[3]
    im = ax3.imshow(conn_pen,vmin=-1, vmax=1)
    ax3.set_title('Pen Hold')
    #ax2.set_xticks(range(conn_mat.shape[0]))
    #ax2.set_yticks(range(conn_mat.shape[1]))
