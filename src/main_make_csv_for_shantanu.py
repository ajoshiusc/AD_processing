...
df2.to_csv('replicate_OASIS_all_subjects_with_mmse_cdr_avg.csv')
...
for s in sub_ids_ucla:
    ...
    #if num_clinical_days == 1:
    #    clinical_day = int(df1['Label'][s][-4:])
    #    diff_days = abs(session_day_ucla[s] - clinical_day)
    #    session_days_clinical = clinical_day
    #    df2['closest_clnical_day'][s] = clinical_day
    #    df2['closest_mmse'][s] = df1['mmse'][s].iloc[0]
    ...
    for day in range(num_clinical_days):
        ...
        print(session_day_ucla[s] - clinical_day)
...
df2.to_csv('replicate_OASIS_all_subjects_with_mmse_cdr_avgs_closest_clinical_day_mmse_cdr.csv')
